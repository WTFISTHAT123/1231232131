--- Settings
    ...
    if ACTIVATION_KEY == 1986217525 or ACTIVATION_KEY == -735324221 then
        util.require_natives("natives-1660775568") 
        util.toast("[ 그타안전대리.com ]\n\n- Developer: 프로젝트#7777\n\nGta5 1.64 / Build 2082")
        util.keep_running()
    else
        util.toast("허가되지 않은 사용자는 루아를 사용할 수 없습니다.")
        util.stop_script()
    end
...
local MONEY_OPTIONS = menu.list(menu.my_root(), "돈 옵션", {}, "추가   ||   제거   ||   반복", function(); end)
    ...
    menu.divider(MONEY_OPTIONS, ScriptName .. "   ||   돈 반복")
        ...
        menu.toggle_loop(MONEY_OPTIONS, "$30만 반복 - 나이트클럽", {}, "$30만을 5초마다 반복합니다.\n\n5분 30초 동안 활성화하면 $2000만이 모입니다.\n\n사용을 위해 나이트클럽 구매가 필요하며, 나이트클럽 금고 문을 열고 활성화하세요.", function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 24045, 300000) -- NIGHTCLUBMAXSAFEVALUE
            SET_INT_GLOBAL(GLOBAL_VALUE + 24041, 300000) -- NIGHTCLUBINCOMEUPTOPOP
    ...
    menu.divider(MONEY_OPTIONS, ScriptName .. "   ||   돈 설정")
        ...
        MONEY_REMOVE = menu.action(MONEY_OPTIONS, "돈 제거", {}, "가능한 설정 값: $0 ~ $20억\n\n중무장 장비를 호출하는 방식으로 돈을 제거합니다. 중무장 장비를 이미 구매한 상태로 변경하기 때문에 구매할 필요는 없습니다.", function(Value)
            ...
            elseif SetMoney > 2000000000 then
                ...
                SET_INT_GLOBAL(GLOBAL_VALUE + 20288, SetMoney) -- -156036296
...
local SPECIAL_CARGO = menu.list(menu.my_root(), "스페셜 패키지 옵션", {}, "값 변경   ||   도구   ||   쿨타임 제거", function(); end)
    ...
    menu.divider(SPECIAL_CARGO, ScriptName .. "   ||   값 변경")
        ...
        menu.slider(SPECIAL_CARGO, "판매값 조정", {"sccargovalue"}, "스페셜 패키지를 팔았을 때 들어오는 돈을 조정합니다.\n\n최대 600만까지 가능합니다.", 0, 6000000, 0, 100000, function(Value)
            SET_INT_GLOBAL(GLOBAL_VALUE + 15788, Value) -- EXEC_CONTRABAND_SALE_VALUE_THRESHOLD1
        end)
    ...
    menu.divider(SPECIAL_CARGO, ScriptName .. "   ||   쿨타임 제거")
        ...
        menu.toggle_loop(SPECIAL_CARGO, "구매", {}, "", function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 15553, 0) -- EXEC_BUY_COOLDOWN
        end, function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 15553, 300000) -- 
        end)

        menu.toggle_loop(SPECIAL_CARGO, "판매", {}, "", function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 15554, 0) -- EXEC_SELL_COOLDOWN
        end, function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 15554, 1800000)
        end)

local nightclub_money = menu.list(menu.my_root(), "부동산 옵션", {}, "값 변경", function(); end)

    menu.divider(nightclub_money, ScriptName .. "   ||   나이트클럽")

        menu.text_input(nightclub_money, "가격 조정", {"ncvalue"}, "나이트클럽 부동산을 팔 때 생기는 절감을 조정합니다.", function(Value)
            set_true = 0
            set_true = tonumber(set_true)
            SetNCValue = tonumber(Value)
            if SetNCValue > 1000000000 then
                menu.trigger_commands("ncvalueactive off")
                util.toast("[ 그타안전대리.com ] 최대 금액 10억을 넘어갈 수 없습니다. 다시 설정해주세요.")
                set_true = 0
            elseif SetNCValue == nil then
                util.toast("[ 그타안전대리.com ] 값이 0보다 작거나 같을 수 없습니다.")
                set_true = 0
            else
                util.toast("[ 그타안전대리.com ] 밑에 활성화 버튼을 눌러주세요.")
                set_true = 1
            end
        end)

        menu.toggle_loop(nightclub_money, "활성화", {"ncvalueactive"}, "나이트클럽 부동산 값을 고정시킵니다.", function()
            if set_true == 1 then
                STATS.STAT_SET_INT(stats.nightclub, (SetNCValue * 2) + 4500000, true)
            else
                util.toast("[ 그타안전대리.com ] 값이 10억을 넘어갑니다. 다시 설정해주세요.")
                menu.trigger_commands("ncvalueactive off")
            end
        end)

local BUNKER = menu.list(menu.my_root(), "벙커 옵션", {}, "값 변경 || 판매", function(); end)

    menu.divider(BUNKER, ScriptName .. "   ||   값 변경")

        menu.slider(BUNKER, "판매값 조정", {"bunkervalue"}, "벙커 창고 물자를 팔았을 때 들어오는 돈을 조정합니다.", 0, 2140000000, 0, 10000000, function(Value)
            SET_INT_GLOBAL(GLOBAL_VALUE + 21531 , 1) -- GR_MANU_CAPACITY
            SET_INT_GLOBAL(GLOBAL_VALUE + 21537, Value) -- GR_MANU_PRODUCT_VALUE
        end)

    menu.divider(BUNKER, ScriptName .. "   ||   쿨타임 제거")

        menu.toggle_loop(BUNKER, "보급 지연 시간", {}, "", function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 21557, 0)
        end, function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 21557, 600)
        end)

    menu.divider(BUNKER, ScriptName .. "   ||   도구")
        
        menu.action(BUNKER, "자동 판매 완료", {}, "벙커 창고 물자 판매를 위한 미션을 자동으로 클리어 해줍니다.", function()
            SET_INT_LOCAL("gb_gunrunning", 1205 + 774, 0)
        end)

        menu.toggle_loop(BUNKER, "보급품 비용 구매 제거", {}, "벙커에서 보급품을 구매할 때 들어가는 비용을 제거해줍니다.", function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 21555, 1000)
            SET_INT_GLOBAL(GLOBAL_VALUE + 21556, 1000)
        end, function()
            SET_INT_GLOBAL(GLOBAL_VALUE + 21555, 15000)
            SET_INT_GLOBAL(GLOBAL_VALUE + 21556, 15000)
        end)
    ---
